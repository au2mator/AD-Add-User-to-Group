...
# Type: New Service
#
# Title: AD - Add User to Group
#
# v 1.0 Initial Release
# v 1.1 Added Stored Credentials
#       see for details: https://click.au2mator.com/PSCreds/?utm_source=github&utm_medium=social&utm_campaign=AD_AddUserToGroup&utm_content=PS1
...
# Code Template V 1.3
#
# URL: https://au2mator.com/add-user-to-group-active-directory-self-service-with-au2mator/?utm_source=github&utm_medium=social&utm_campaign=AD_AddUserToGroup&utm_content=PS1
...
[string]$LogPath = "C:\_SCOworkingDir\TFS\PS-Services\AD - Add User to Group"
[string]$LogfileName = "Add User to Group"

[string]$CredentialStorePath = "C:\_SCOworkingDir\TFS\PS-Services\CredentialStore" #see for details: https://click.au2mator.com/PSCreds/?utm_source=github&utm_medium=social&utm_campaign=AD_AddUserToGroup&utm_content=PS1
...
# Stored Credentials
# See: https://click.au2mator.com/PSCreds/?utm_source=github&utm_medium=social&utm_campaign=AD_AddUserToGroup&utm_content=PS1
...
if (Get-ADGroupMember -Identity $c_Group | Where-Object -Property DistinguishedName -Value $c_User -EQ) {
    ...
    $au2matorReturn = "User $((Get-ADUser -identity $c_User).Name) is already a Member of the Group $((Get-ADGroup -Identity $c_Group).Name)"
    $AdditionalHTML = "<br>
    User $((Get-ADUser -identity $c_User).Name) is already a Member of the Group $((Get-ADGroup -Identity $c_Group).Name)
    <br>
    "
    $Status = "COMPLETED"
}
    ...
    try {
        ...
        $au2matorReturn = "User $((Get-ADUser -identity $c_User).Name) added in Group $((Get-ADGroup -Identity $c_Group).Name)"
        $AdditionalHTML = "<br>
        User $((Get-ADUser -identity $c_User).Name) added in Group $((Get-ADGroup -Identity $c_Group).Name)
        <br>
        "
        $Status = "COMPLETED"
    }
        ...
        $au2matorReturn = "User $((Get-ADUser -identity $c_User).Name) failed to add in Group $((Get-ADGroup -Identity $c_Group).Name), Error: $Error"
        $AdditionalHTML = "<br>
        User $((Get-ADUser -identity $c_User).Name) failed to add in Group $((Get-ADGroup -Identity $c_Group).Name)
        <br>
        Error: $Error
        "
        $Status = "ERROR"
